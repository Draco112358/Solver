...
function compute_ha(xo::Float64, yo::Float64, zo::Float64,
    ...
    # --- Pre-allocazione dei vettori temporanei QUI ---
    # Questi vettori avranno dimensione 3 (come rmi, rai, etc.)
    # Vengono allocati una sola volta per chiamata a compute_ha
        ...
        @inbounds for b1 = 1:nlky
            sum_c1 = 0.0 + im*0.0
             @inbounds for c1 = 1:nlkz
                ...
                # --- Operazioni in-place usando .= ---
                # Questo aggiorna _drai_temp invece di allocarne uno nuovo
                _drai_temp .= rai .+ rabi .* dky_val .+ raci .* dkz_val .+ rabci .* dky_val .* dkz_val
                _drbi_temp .= rbi .+ rabi .* dkx_val .+ rbci .* dkz_val .+ rabci .* dkx_val .* dkz_val
                _drci_temp .= rci .+ raci .* dkx_val .+ rbci .* dky_val .+ rabci .* dkx_val .* dky_val
                
                draim = norm(_drai_temp, 2)
                drbim = norm(_drbi_temp, 2)
                drcim = norm(_drci_temp, 2)
                
                _r1_temp .= rmi .+ rai .* dkx_val .+ rbi .* dky_val .+ rci .* dkz_val .+ 
                            rabi .* dkx_val .* dky_val .+ raci .* dkx_val .* dkz_val .+ 
                            rbci .* dky_val .* dkz_val .+ rabci .* dkx_val .* dky_val .* dkz_val
