function FFT_solver_QS_S_type(freq, escalings, incidence_selection, FFTCP, FFTCLp, diagonals, ports, ports_scatter_value, lumped_elements, expansions, GMRES_settings, Zs_info, QS_Rcc_FW, id, chan, commentsEnabled::Bool)
    ...
    if !isnothing(checkpoint_gmres) && haskey(checkpoint_gmres, :k) && haskey(checkpoint_gmres, :c1)
        println("Resuming FFT_solver_QS_S_type from checkpoint (freq index: $(checkpoint_gmres[:k]), port index: $(checkpoint_gmres[:c1]))")
    ...
    for k = start_k:nfreq
        if QS_Rcc_FW == 2
            ...
            PLIVector = []
            for cont = 1:3
                Nx::Int64 = size(FFTCLp_rebuilted[cont, 1], 1) ÷ 2
                Ny::Int64 = size(FFTCLp_rebuilted[cont, 1], 2) ÷ 2
                Nz::Int64 = size(FFTCLp_rebuilted[cont, 1], 3) ÷ 2
                padded_CircKt = zeros(ComplexF64, 2 * Nx, 2 * Ny, 2 * Nz)
                push!(PVector, plan_fft!(padded_CircKt, flags=FFTW.MEASURE))
                push!(PLIVector, plan_ifft!(FFTCLp_rebuilted[cont, 1], flags=FFTW.MEASURE))
                # push!(ChiVector, padded_CircKt)
            end
            ...
            for cont1 = 1:3
                for cont2 = cont1:3
                    ...
                    Ny::Int64 = size(FFTCP_rebuilted[cont1, cont2], 2) ÷ 2
                    Nz::Int64 = size(FFTCP_rebuilted[cont1, cont2], 3) ÷ 2
                    padded_CircKt = zeros(ComplexF64, 2 * Nx, 2 * Ny, 2 * Nz)
                    #Chi = ifft(FFTCP_rebuilted[cont1, cont2] .* fft(padded_CircKt))
                    if cont1 != cont2
                        P2Vector[cont1, cont2] = plan_fft(padded_CircKt, flags=FFTW.MEASURE)
                        Chi2Vector[cont1, cont2] = padded_CircKt
                    else
                    ...
                    # P2Vector[cont1, cont2] = plan_fft(padded_CircKt, flags=FFTW.MEASURE)
                    PLI2Vector[cont1, cont2] = plan_ifft!(FFTCP_rebuilted[cont1, cont2], flags=FFTW.MEASURE)
                    # Chi2Vector[cont1, cont2] = padded_CircKt
                end
            end
            # elseif QS_Rcc_FW==3
        ...
        for c1::Int64 = c1_start_val:size(ports["port_nodes"], 1)
            ...
            if QS_Rcc_FW == 1
                ...
                #println("start gmres")
                V, flag, relres, iter, resvec = gmres_custom(tn, false, GMRES_settings["tol"][k], Inner_Iter, Vrest[:, c1], w[k], incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, F, PLIVector, PVector, PLI2Vector, P2Vector, Chi2Vector, id, chan, c1, checkpoint_cb, checkpoint_interval)
                if flag == 99
                    return nothing
                end
                ...
                V, flag, relres, iter, resvec = gmres_custom(tn, false, GMRES_settings["tol"][k], Inner_Iter, Vrest[:, c1], w[k], incidence_selection, FFTCP_rebuilted, FFTCLp_rebuilted, DZ, Yle, expansions, invZ, invP, F, PLIVector, PVector, PLI2Vector, P2Vector, Chi2Vector, id, chan, c1, checkpoint_cb, checkpoint_interval)
                if flag == 99
                    return nothing
                end
