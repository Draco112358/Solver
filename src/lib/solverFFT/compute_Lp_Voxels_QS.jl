using MKL
# Define the round_ud function if it is not already available
function round_ud(arr, precision)
    factor = 10^precision
    ...
    elseif sum_supp == 2  # line-volume or surface-surface
        ...
        elseif is_line_v2 == 1  # bar2 is a line
            ...
            if supp_x1 == 1  # bar1 is a surface in y-z plane
                ...
                elseif supp_y2 == 1  # bar2 is a surface in x-z plane
                    integ = a1 * b2 * integ_surf_surf_ortho([x2v[1], x2v[end]], [z2v[1], z2v[end]], yc2, [y1v[1], y1v[end]], [z1v[1], z1v[end]], xc1)
                else  # bar2 is a surface in x-y plane
                    integ = a1 * c2 * integ_surf_surf_ortho([x2v[1], x2v[end]], [y2v[1], y2v[end]], zc2, [y1v[1], y1v[end]], [z1v[1], z1v[end]], xc1)
                end
            elseif supp_y1 == 1  # bar1 is a surface in x-z plane
                if supp_x2 == 1  # bar2 is a surface in y-z plane
                    integ = b1 * a2 * integ_surf_surf_ortho([x1v[1], x1v[end]], [z1v[1], z1v[end]], yc1, [y2v[1], y2v[end]], [z2v[1], z2v[end]], xc2)
                elseif supp_y2 == 1  # bar2 is a surface in x-z plane
                    integ = b1 * b2 * integ_surf_surf_para([x1v[1], x1v[end]], [z1v[1], z1v[end]], yc1, [x2v[1], x2v[end]], [z2v[1], z2v[end]], yc2)
                else  # bar2 is a surface in x-y plane
                    integ = b1 * c2 * integ_surf_surf_ortho([x2v[1], x2v[end]], [y2v[1], y2v[end]], zc2, [x1v[1], x1v[end]], [z1v[1], z1v[end]], yc1)
                end
            else  # bar1 is a surface in x-y plane
                if supp_x2 == 1  # bar2 is a surface in y-z plane
                    integ = c1 * a2 * integ_surf_surf_ortho([x1v[1], x1v[end]], [y1v[1], y1v[end]], zc1, [y2v[1], y2v[end]], [z2v[1], z2v[end]], xc2)
                elseif supp_y2 == 1  # bar2 is a surface in x-z plane
                    integ = c1 * b2 * integ_surf_surf_ortho([x1v[1], x1v[end]], [y1v[1], y1v[end]], zc1, [x2v[1], x2v[end]], [z2v[1], z2v[end]], yc2)
                else  # bar2 is a surface in x-y plane
...
function check_condition(eps1, eps2, eps3, V1, V2, max_d, min_R, size_dim, other_dim1, other_dim2)
    # Calculate the maximum of the two other dimensions
    max_oth = max(other_dim1, other_dim2)

    # Calculate the condition variables
    condX1a = V1 * V2 * max_d / (min_R + 1e-15)^3
    condX1f = size_dim / (min_R + 1e-15)
    condX1b = size_dim / max_oth

    # Initialize supp_dim to 0
    supp_dim = 0

    # Check conditions and update supp_dim
...
# Function to compute the integral for point source to point source
...
# Function to compute the integral for line source to line source
