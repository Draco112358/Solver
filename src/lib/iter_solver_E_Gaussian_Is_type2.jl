...
function iter_solver_E_Gaussian_Is_type2(
	freq,
	escalings,
	incidence_selection,
	P_data,
	...
	if state.currentState != "compute fields"
		...
		state.intermediateResults["indx"] = 1:mx
		state.intermediateResults["indy"] = mx+1:mx+my
		state.intermediateResults["indz"] = mx+my+1:mx+my+mz

		m = mx + my + mz
		state.intermediateResults["m"] = m
		n::Int64 = size(incidence_selection[:A], 2)
		state.intermediateResults["n"] = n
		ns::Int64 = size(incidence_selection[:Gamma], 2)
		state.intermediateResults["ns"] = ns
		w = 2 .* pi .* freq
		nfreq = length(w)
		...
		state.intermediateResults["out"] = out
		Vrest = zeros(m + n + ns)
		...
		state.intermediateResults["hc"] = []
		state.intermediateResults["hc_3D"] = []
		state.intermediateResults["ha"] = []
		state.intermediateResults["ha_3D"] = []
		state.intermediateResults["Lambda_x"] = []
		state.intermediateResults["Lambda_y"] = []
		state.intermediateResults["Lambda_z"] = []
		state.escalings = escalings
		V = []
		state.intermediateResults["nfreq"] = nfreq
		...
		for k ∈ 1:nfreq
			...
			Res, flag, relres, iter, resvec = gmres_custom_new2(tn, false, GMRES_settings["tol"][k], Inner_Iter, Vrest, w[k], incidence_selection, P_rebuilted, Lp_rebuilted, Z_self, Yle, invZ, invP, F, resProd, id, chan, 1)
			if flag == 99
				return false
			end
			...
			for c1 in 1:size(ports[:port_nodes], 1)
				n1::Int64 = convert(Int64, ports[:port_nodes][c1, 1])
				n2::Int64 = convert(Int64, ports[:port_nodes][c1, 2])
				out["Vp"][c1, k] = Res[m+ns+n1] - Res[m+ns+n2]
			end
			push!(V, Res)
		end
		state.intermediateResults["V"] = V
		state.intermediateResults["eps0"] = eps0
		state.intermediateResults["mu0"] = mu0
		state.intermediateResults["unitario"] = unitario
		state.intermediateResults["zeroo"] = zeroo
		state.intermediateResults["ordine_int"] = ordine_int
		state.currentState = "compute fields"
		state.intermediateResults["k"] = 1
		save_task_state(state, aws_config, bucket_name, "electric_fields")
	end
	if state.currentState == "compute fields"
		for k ∈ state.intermediateResults["k"]:state.intermediateResults["nfreq"]
			state.intermediateResults["k"] = k
			I = state.intermediateResults["V"][k][1:state.intermediateResults["m"]] ./ state.escalings[:Is]
			J = I ./ state.volumi[:S]
			...
			state.superfici = superfici
			state.escalings = escalings
			state.intermediateResults["freqk"] = freq[k]
			sigma = (state.intermediateResults["V"][k][state.intermediateResults["m"]+1:state.intermediateResults["m"]+state.intermediateResults["ns"]] ./ state.superfici["S"]) / state.escalings[:Cd]
			beta = 2 * pi * state.intermediateResults["freqk"] / state.escalings[:freq] * sqrt(state.intermediateResults["eps0"] * state.intermediateResults["mu0"])
			state.currentState = "compute hc"
			#save_task_state(state, aws_config, bucket_name, "electric_fields")
			if state.currentStateComputeFields == "compute hc"
				hc = compute_Ec_Gauss(state.superfici["estremi_celle"], state.superfici["normale"], centri_oss, state.intermediateResults["ordine_int"], beta, id, chan)
				if hc == false
					return false
				end
				println("hc")
				...
				push!(state.intermediateResults["hc"], hc)
				hc_3D = compute_Ec_Gauss(state.superfici["estremi_celle"], state.superfici["normale"], centri_oss_3D, state.intermediateResults["ordine_int"], beta, id, chan)
				if hc_3D == false
					return false
				end
				println("hc_3D")
				...
				push!(state.intermediateResults["hc_3D"], hc_3D)
				state.currentState = "compute fields"
				state.currentStateComputeFields = "compute ha"
				save_task_state(state, aws_config, bucket_name, "electric_fields")
			end
			
			if state.currentStateComputeFields == "compute ha"
				ha = compute_Ar_Gauss(state.volumi[:coordinate], centri_oss, state.intermediateResults["ordine_int"], beta, id, chan)
				if ha == false
					return false
				end
				println("ha")
				...
				push!(state.intermediateResults["ha"], ha)
				ha_3D = compute_Ar_Gauss(state.volumi[:coordinate], centri_oss_3D, state.intermediateResults["ordine_int"], beta, id, chan)
				if ha_3D == false
					return false
				end
				println("ha_3D")
				...
				push!(state.intermediateResults["ha_3D"], ha_3D)
				state.currentState = "compute fields"
				state.currentStateComputeFields = "compute Lambda_x"
				save_task_state(state, aws_config, bucket_name, "electric_fields")
			end
			
			if state.currentStateComputeFields == "compute Lambda_x"
				Lambda_x = compute_lambda_numeric(centri_oss_3D, state.volumi, state.incidence_selection, [state.intermediateResults["unitario"] state.intermediateResults["zeroo"] state.intermediateResults["zeroo"]], state.intermediateResults["ordine_int"], beta, id, chan)
				if Lambda_x == false
					return false
				end
				println("Lambda_x")
				...
				push!(state.intermediateResults["Lambda_x"], Lambda_x)
				state.currentState = "compute fields"
				state.currentStateComputeFields = "compute Lambda_y"
				save_task_state(state, aws_config, bucket_name, "electric_fields")
			end
			
			if state.currentStateComputeFields == "compute Lambda_y"
				Lambda_y = compute_lambda_numeric(centri_oss_3D, state.volumi, state.incidence_selection, [state.intermediateResults["zeroo"] state.intermediateResults["unitario"] state.intermediateResults["zeroo"]], state.intermediateResults["ordine_int"], beta, id, chan)
				if Lambda_y == false
					return false
				end
				println("Lambda_y")
				...
				push!(state.intermediateResults["Lambda_y"], Lambda_y)
				state.currentState = "compute fields"
				state.currentStateComputeFields = "compute Lambda_z"
				save_task_state(state, aws_config, bucket_name, "electric_fields")
			end
			
			if state.currentStateComputeFields == "compute Lambda_z"
				Lambda_z = compute_lambda_numeric(centri_oss_3D, state.volumi, state.incidence_selection, [state.intermediateResults["zeroo"] state.intermediateResults["zeroo"] state.intermediateResults["unitario"]], state.intermediateResults["ordine_int"], beta, id, chan)
				if Lambda_z == false
					return false
				end
				println("Lambda_z")
				...
				push!(state.intermediateResults["Lambda_z"], Lambda_z)
				state.currentState = "compute fields"
				state.currentStateComputeFields = k == state.intermediateResults["nfreq"] ? "" : "compute hc"
				save_task_state(state, aws_config, bucket_name, "electric_fields")
			end
			
	
			state.intermediateResults["out"]["Ex"][:, k], state.intermediateResults["out"]["Ey"][:, k], state.intermediateResults["out"]["Ez"][:, k] = compute_E_field_Gauss(state.intermediateResults["indx"], state.intermediateResults["indy"], state.intermediateResults["indz"], centri_oss, state.intermediateResults["hc"][k], state.intermediateResults["ha"][k], J, sigma, state.intermediateResults["freqk"] / state.escalings[:freq])
			state.intermediateResults["out"]["Ex_3D"][:, k], state.intermediateResults["out"]["Ey_3D"][:, k], state.intermediateResults["out"]["Ez_3D"][:, k] = compute_E_field_Gauss(state.intermediateResults["indx"], state.intermediateResults["indy"], state.intermediateResults["indz"], centri_oss_3D, state.intermediateResults["hc_3D"][k], state.intermediateResults["ha_3D"][k], J, sigma, state.intermediateResults["freqk"] / state.escalings[:freq])
			state.intermediateResults["out"]["Hx_3D"][:, k], state.intermediateResults["out"]["Hy_3D"][:, k], state.intermediateResults["out"]["Hz_3D"][:, k] = compute_H_field_Gauss(state.intermediateResults["Lambda_x"][k], state.intermediateResults["Lambda_y"][k], state.intermediateResults["Lambda_z"][k], I)
		end
	end
	
	return state.intermediateResults["out"]
end
