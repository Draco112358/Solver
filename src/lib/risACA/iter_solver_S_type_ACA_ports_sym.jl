function iter_solver_S_type_ACA_ports_sym(freq, escalings, incidence_selection, P_data, Lp_data, ports, lumped_elements, GMRES_settings, volumi, superfici, use_Zs_in, QS_Rcc_FW, ACA_thres, ports_to_excite, mat_imp_simm, id, chan, commentsEnabled)
    ...
    for k = start_k:nfreq
        ...
        for c1 = c1_start_val:length(ports_to_excite)
            ...
            n2 = Int64(ports[:port_nodes][idx_port, 2])
            ...
            for c2 = c1:size(ports[:port_nodes], 1)
                ...
                # Careful with 1-based indexing if V has size m+n+ns

                if ports_to_excite[c1] == c2
                    S[ports_to_excite[c1], c2, k] = (2 * (V[ind1] - V[ind2]) - R_chiusura) / R_chiusura
                else
                    S[ports_to_excite[c1], c2, k] = (2 * (V[ind1] - V[ind2])) / R_chiusura
                end
        ...
        for kk = range(1, size(mat_imp_simm, 1))
            S[Int64(mat_imp_simm[kk, 1]), Int64(mat_imp_simm[kk, 2]), k] = S[Int64(mat_imp_simm[kk, 3]), Int64(mat_imp_simm[kk, 4]), k]
            S[Int64(mat_imp_simm[kk, 2]), Int64(mat_imp_simm[kk, 1]), k] = S[Int64(mat_imp_simm[kk, 3]), Int64(mat_imp_simm[kk, 4]), k]
        end
