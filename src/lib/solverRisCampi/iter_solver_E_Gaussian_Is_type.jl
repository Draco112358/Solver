...
function iter_solver_E_Gaussian_Is_type(
	...
	for k ∈ 1:nfreq
		...
		V, flag, relres, iter, resvec = @time gmres_custom!(out_gmres, work, pc_work, tn, false, GMRES_settings["tol"][k], Inner_Iter, ComplexF64.(Vrest), w[k], incidence_selection, P_data, Lp_data, Z_self, Yle, invZ, invP, F, id, chan, 1)
		if flag == 99
			return nothing
		end
