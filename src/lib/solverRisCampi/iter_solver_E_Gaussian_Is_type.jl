    ...
    for k ∈ 1:nfreq
        ...
        @time begin
            ...
            if !isempty(volumi[:indici_dielettrici])
                invCd[Int64.(volumi[:indici_dielettrici])] .= 1 ./ (1im * w * volumi[:Cd][Int64.(volumi[:indici_dielettrici])])
            end

            # Check use_Zs_in and compute Z_self
            if use_Zs_in == 1
                # Compute Zs
                Zs = real.(sqrt(1im * w / escalings[:freq]) .* volumi[:Zs_part])
            ...
            invZ = sparse(1:m, 1:m, 1 ./ (Z_self + 1im * w * diag_Lp), m, m)
            # --------------------- preconditioner ------------------------
            SS::SparseArrays.SparseMatrixCSC{ComplexF64,Int64} = Yle + (transpose(incidence_selection[:A]) * (invZ * incidence_selection[:A])) + 1im * w * (incidence_selection[:Gamma] * invP) * transpose(incidence_selection[:Gamma])
...
function handle_scaling_and_rebuilding!(
    ...
    if QS_Rcc_FW == 2
        ...
        @threads for i in eachindex(Lpx)
            Lpx[i] *= cis(-β * Rx[i])
        end
        @threads for i in eachindex(Lpy)
            Lpy[i] *= cis(-β * Ry[i])
        end
        @threads for i in eachindex(Lpz)
            Lpz[i] *= cis(-β * Rz[i])
        end
        @threads for i in eachindex(Pm)
            Pm[i] *= cis(-β * Rcc[i])
        end
    end
